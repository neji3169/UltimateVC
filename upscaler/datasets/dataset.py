...
def amp_pha_stft(audio, n_fft, hop_size, win_size, center=True):
    ...
    # Ensure audio is 1D or 2D for stft
    if audio.ndim > 2:
        # Squeeze the likely extra channel dimension, e.g., [1, 1, samples] -> [1, samples]
        audio_squeezed = audio.squeeze(1)
        audio = audio_squeezed
    elif audio.ndim == 1:
         # Add batch dim if input is 1D [samples] -> [1, samples]
         audio_batched = audio.unsqueeze(0)
         audio = audio_batched
...
def amp_pha_istft(log_amp, pha, n_fft, hop_size, win_size, center=True, output_length=None): # Added output_length
    
    amp = torch.exp(log_amp)
    com = torch.complex(amp*torch.cos(pha), amp*torch.sin(pha))
    hann_window = torch.hann_window(win_size).to(com.device)
    # Pass output_length to torch.istft if provided
    audio = torch.istft(com, n_fft, hop_length=hop_size, win_length=win_size, window=hann_window, center=center, length=output_length)
